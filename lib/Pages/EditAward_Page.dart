...
class EditAward extends StatefulWidget {
  var IdResponse, UserNameResponse;
  EditAward(this.IdResponse, this.UserNameResponse);
  EditAward.none();

  @override
  State<StatefulWidget> createState() {
    return EditAwardState();
  }
}

class EditAwardState extends State<EditAward> {
  final TypeTextBox = TextEditingController();
  final AmountTextBox = TextEditingController();
  var Id, Type, Amount;
  var appscaffold;
  Future loadfuture;
  var awardsInformation;
              ...
              child: Stack(
                  children: [
                    ...
                    Padding(padding: EdgeInsets.only(right: 730, top: 80),
                      child: new Text("ایجاد جایزه", style: new TextStyle(color: Colors.deepPurple, fontWeight: FontWeight.bold, fontFamily: 'IRANSans', fontSize: 25)),
                    ),// عنوان صفحه
                    Padding(padding: EdgeInsets.only(right: 545, top: 283),
                    ...
                    Padding(padding: EdgeInsets.only(left: 400,right: 580, top: 280),
                      child: Card(
                            ...
                            children: <Widget>[
                              new Flexible(
                                  ...
                                  child: new Padding(padding: EdgeInsets.fromLTRB(10, 14, 10, 0),
                                      child: TextField(inputFormatters: <TextInputFormatter>[LengthLimitingTextInputFormatter(40)], style: TextStyle(color: Colors.white),controller: TypeTextBox, textAlign: TextAlign.center, cursorColor: Colors.white,
                    ...
                    Padding(padding: EdgeInsets.only(right: 545, top: 333),
                      child: new Text("مبلغ", style: new TextStyle(color: Colors.deepPurple, fontWeight: FontWeight.normal, fontFamily: 'IRANSans', fontSize: 20)),
                    ),// مبلغ
                    Padding(padding: EdgeInsets.only(left: 400,right: 580, top: 330),
                      child: Card(
                        color: Color(0xff9370DB),
                        child: Container(
                          width: width,
                          ...
                          child: Row(
                            ...
                            children: <Widget>[
                              new Flexible(
                                  fit: FlexFit.loose,
                                  child: new Padding(padding: EdgeInsets.fromLTRB(10, 14, 10, 0),
                                      child: TextField(inputFormatters: <TextInputFormatter>[LengthLimitingTextInputFormatter(40)], style: TextStyle(color: Colors.white),controller: AmountTextBox, textAlign: TextAlign.center, cursorColor: Colors.white,
                                        decoration: new InputDecoration(border: InputBorder.none, hintText: "", hintStyle: TextStyle(color: Colors.grey)),
                                        keyboardType: TextInputType.multiline,
                                        autofocus: true,
                                      ))),
                    ...
                    Padding(padding: EdgeInsets.only(right: 710, top: 435),
                      child: InkWell(
                        ...
                        onTap: (){
                          Navigator.of(context).pushAndRemoveUntil(
                              MaterialPageRoute(builder: (context) =>
                              new Directionality(textDirection: TextDirection.rtl, child: Awards(UserName.toString()))),(Route<dynamic> route) => false);
                        },
                      ),
                    ), // بازگشت
                  ]),
  ...
  GetAwardById() async{
    print('GetAwardById Run...');
    Id = widget.IdResponse;
    try {
      FormData formData = FormData.fromMap({
        "id":Id,
      });
      Response response = await Dio().post("http://jamq.ir:3000/Mainapp/GetAwardById",options: Options(contentType: 'multipart/form-data'),data:formData);
      if(response.data.toString() == 'Awards Does Not Exist!!!'){
          ...
          buttons: [
            DialogButton(
              ...
              onPressed: () => Navigator.of(context).pushAndRemoveUntil(
                  MaterialPageRoute(builder: (context) =>
                  new Directionality(textDirection: TextDirection.rtl, child: AbouteUsList(UserName.toString()))),(Route<dynamic> route) => false),
              color: Color(0xffD3D3D3),
            )
          ],
        ).show(); // Message
        ...
        Type = awardsInformation["AwType"];
        Amount = awardsInformation["AwAmount"];
        FillInfo();
  ...
  EditAward() async {
    print('EditAward Run...');
    if(TypeTextBox.text == '' || AmountTextBox.text == ''){
      Alert(
        context: context,
        type: AlertType.none,
        title: "پیغام",
      ...
      try {
        ...
        Response response = await Dio().post("http://jamq.ir:3000/Mainapp/EditAward",options: Options(contentType: 'multipart/form-data'),data:formData);
        print(response.data.toString());
        if(response.data.toString() == 'EditAward Done!') {
