...
class UnauthorizedWords extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return UnauthorizedWordsState();
  }
}

class UnauthorizedWordsState extends State<UnauthorizedWords> {
  final SearchTextBox = TextEditingController();
  var appscaffold;
  Future loadfuture;
  List cursedwordsInformation;
  bool IsSearch = false;

  @override
  void initState() {
    super.initState();
    loadfuture = GetCursedWords();
  }
              ...
              child: Stack(
                  children: [
                    Padding(padding: EdgeInsets.only(left: 10),
                      child: Row(
                        children: [
                          Padding(padding: EdgeInsets.only(right: 1100, top: 160),
                            child:
                            Container(
                              width: 100,
                              height: 30,
                          ...
                          Padding(padding: EdgeInsets.only(right: 10, top: 160),
                            child:
                            Container(
                              width: 100,
                              height: 30,
                    ...
                    Padding(padding: EdgeInsets.only(right: 705, top: 80),
                      child: new Text("کلمات غیر مجاز", style: new TextStyle(color: Colors.deepPurple, fontWeight: FontWeight.bold, fontFamily: 'IRANSans', fontSize: 25)),
                    ),// عنوان صفحه
                    Padding(padding: EdgeInsets.only(right: 490, top: 160),
                      child: InkWell(
                        ...
                        onTap: (){
                          cursedwordsInformation.clear();
                          GetCursedWordsSearch();
                        },
                      ),
                    ), // دکمه جستجو
                    Padding(padding: EdgeInsets.only(left: 400,right: 600, top: 150),
                      child: Card(
                            ...
                            children: <Widget>[
                              new Flexible(
                                  ...
                                  child: new Padding(padding: EdgeInsets.fromLTRB(0, 18, 0, 0),
                                      ...
                                      child: TextField(inputFormatters: <TextInputFormatter>[LengthLimitingTextInputFormatter(30), BlacklistingTextInputFormatter.singleLineFormatter], style: TextStyle(color: Colors.white),controller: SearchTextBox, textAlign: TextAlign.center, cursorColor: Colors.white,
                        ...
                        Card(
                          child: Container(
                            ...
                            Column(
                                children: [
                                  Row(children: [
                                    Padding(padding: EdgeInsets.only(right: 135, top: 5),
                                      child: new Text("کلمه غیر مجاز", style: new TextStyle(color: Colors.white, fontWeight: FontWeight.normal, fontSize: 15)),
                                    ),
                                  ]),
                                ]),
                          ),
                    ...
                    Padding(padding: EdgeInsets.only(left: 400,right: 598, top: 280),
                      child: Container(
                        ...
                        child: new ListView.builder(
                            ...
                            shrinkWrap: true,
                            itemCount: cursedwordsInformation.length,
                                        ...
                                        Padding(padding:EdgeInsets.only(right: 28),
                                          child: Container(
                                            ...
                                            child: Padding(padding: EdgeInsets.only(right: 0),
                                              child: Center(
                                                child: new Text(cursedwordsInformation[index]["CWCursedWord"].toString(), style: new TextStyle(color: Color(0xff2E0273), fontFamily: 'IRANSans', fontWeight: FontWeight.bold, fontSize: 13)),
                                              ),
                                            ),
                                          ),
                                        ),
  ...
  GetCursedWords() async{
    print('GetCursedWords Run...');
    try {
      Response response = await Dio().post("http://jamq.ir:3000/Mainuser/GetCursedWords");
      if(response.data.toString() == 'CursedWords Does Not Exist!!!'){
        Alert(
          context: context,
          type: AlertType.none,
          title: "پیغام",
          desc: "!!!کلمه ای وجود ندارد",
          buttons: [
            DialogButton(
              child: Text(
                "تایید",
        ...
        cursedwordsInformation = response.data;
        print('cursedwordsInformation = '+cursedwordsInformation.toString());
        return cursedwordsInformation;
      }
    } catch (e) {
      Alert(
        context: context,
  ...
  GetCursedWordsSearch() async{
    print('GetCursedWordsSearch Run...');
    IsSearch = true;
    if(SearchTextBox.text == ''){
      Alert(
        ...
        type: AlertType.none,
        title: "پیغام",
        desc: "!!!لطفا کلمه مورد نظر را وارد کنید",
        buttons: [
          DialogButton(
            child: Text(
              "تایید",
      ...
      try {
        FormData formData = FormData.fromMap({
          "CursedWord":SearchTextBox.text,
        });
        _openLoadingDialog(context);
        Response response = await Dio().post("http://jamq.ir:3000/Mainuser/GetCursedWordsSearch",options: Options(contentType: 'multipart/form-data'),data:formData);
        if(response.data.toString() != 'CursedWords Does Not Exist!!!'){
