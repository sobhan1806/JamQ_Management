...
import 'FullTextTicket_Page.dart';
...
class SendResponseTicket extends StatefulWidget {
  var UserIdResponse, UserNameResponse;
  SendResponseTicket(this.UserIdResponse, this.UserNameResponse);
  SendResponseTicket.none();

  @override
  State<StatefulWidget> createState() {
    return SendResponseTicketState();
  }
}

class SendResponseTicketState extends State<SendResponseTicket> {
  final UserIdTextBox = TextEditingController();
  final TextTextBox = TextEditingController();
  var UserId, IsOpened;
  var appscaffold;
  Future loadfuture;
  List ticketInformation;
              ...
              child: Stack(
                  children: [
                    ...
                    Padding(padding: EdgeInsets.only(right: 690, top: 80),
                      child: new Text("ارسال پاسخ تیکت", style: new TextStyle(color: Colors.deepPurple, fontWeight: FontWeight.bold, fontFamily: 'IRANSans', fontSize: 25)),
                    ),// عنوان صفحه
                    ...
                    Padding(padding: EdgeInsets.only(right: 410, top: 213),
                    ...
                    Padding(padding: EdgeInsets.only(right: 640, top: 500),
                      child: InkWell(
                        child: Container(
                          width: 100,
                          height: 30,
                    ...
                    Padding(padding: EdgeInsets.only(right: 775, top: 500),
                      child: InkWell(
                        child: Container(
                          width: 100,
                          height: 30,
                        ...
                        onTap: (){
                          Navigator.of(context).pushAndRemoveUntil(
                              MaterialPageRoute(builder: (context) =>
                              new Directionality(textDirection: TextDirection.rtl, child: FullTextTicket(UserId, UserName.toString()))),(Route<dynamic> route) => false);
                        },
                      ),
                    ), // بازگشت
                  ]),
  ...
  GetTicketById() async{
    ...
    try {
      ...
      if(response.data.toString() != 'Ticket Does Not Exist!!!'){
        ticketInformation = response.data;
        print('GetTicketById = '+ticketInformation.toString());
        FillInfo();
  ...
  SendResponseTicket() async {
    ...
    try {
      ...
      UserId = widget.UserIdResponse;
      FormData formData = FormData.fromMap({
        "UserId":UserId,
        "Response":TextTextBox.text,
        "Operator":UserName.toString(),
      });
      _openLoadingDialog(context);
      print('Response Run');
      Response response = await Dio().post("http://jamq.ir:3000/Ticket/SendResponseTickets",options: Options(contentType: 'multipart/form-data'),data:formData);
      print(response.data.toString());
      if(response.data.toString() == "SendResponseTickets Done!") {
        Navigator.pop(context);
        Alert(
          context: context,
          type: AlertType.none,
          title: "پیغام",
          desc: ".پاسخ با موفقیت ارسال شد",
          buttons: [
            DialogButton(
              child: Text(
                "تایید",
